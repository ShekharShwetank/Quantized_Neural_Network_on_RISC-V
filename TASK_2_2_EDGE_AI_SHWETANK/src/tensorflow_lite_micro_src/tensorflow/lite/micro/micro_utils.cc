...
size_t EvalTensorBytes(const TfLiteEvalTensor* eval_tensor) {
  if (eval_tensor->type == kTfLiteString) {
    // String tensors are handled specially; their size depends on content,
    // not just type and dimensions. This minimal implementation assumes fixed size,
    // which may need refinement for full string support.
    return 0; // Or a symbolic size if strings are allocated dynamically.
  }
  return TfLiteTypeGetSize(eval_tensor->type) * ElementCount(*eval_tensor->dims);
}
